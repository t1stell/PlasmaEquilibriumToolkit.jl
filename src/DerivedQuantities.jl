...
function B_norm(::Contravariant, e::BasisVectors)
  return norm(cross(e[:, 1], e[:, 2]), 2)
end

function B_norm(::Covariant, e::BasisVectors, ι::Real)
...
function B_field(::Contravariant, e::BasisVectors)
  return cross(e[:, 1], e[:, 2])
end

function B_field(::Covariant, e::BasisVectors, ι::Real)
...
function B_norm(::Contravariant, e::AbstractArray{BasisVectors{T}};) where {T}
...
function B_norm!(
  ...
  size(B) == size(e) ||
    throw(DimensionMismatch("Incompatible sizes in B_norm!"))
...
function B_field(::Contravariant, e::AbstractArray{BasisVectors{T}};) where {T}
...
function B_field!(
  ...
  size(B) == size(e) ||
    throw(DimensionMismatch("Incompatible sizes in B_field!"))
  @batch minbatch = 16 for i in eachindex(B, e)
...
function B_norm!(
  ...
  size(B) == size(x) ||
    throw(DimensionMismatch("Incompatible dimensions in B_norm!"))
