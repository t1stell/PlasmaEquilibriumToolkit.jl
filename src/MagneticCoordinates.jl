...
"""
    BoozerCoordinates{T,A}(ψ::T,θ::A,ζ::A)
...
function BoozerCoordinates(ψ,χ,ζ)
  ψ2, χ2, ζ2 = promote(ψ,χ,ζ)
  return BoozerCoordinates{typeof(ψ2),typeof(χ2)}(ψ2,χ2,ζ2)
end

function BoozerCoordinates(ψ,χ,ζ::T) where T <: AbstractVector
  coords = Vector{BoozerCoordinates}(undef,length(ζ))
  map!(z->BoozerCoordinates(ψ,χ,z),coords,ζ)
  return coords
end

function BoozerCoordinates(ψ,χ::AbstractVector,ζ::AbstractVector)
  coords = Matrix{BoozerCoordinates}(undef,length(ζ),length(χ))
  for x = 1:length(χ)
    for z = 1:length(ζ)
      @inbounds coords[z,x] = BoozerCoordinates(ψ,χ[x],ζ[z])
    end
  end
  return coords
end

function BoozerCoordinates(ψ,χ::AbstractMatrix,ζ::AbstractMatrix)
  @assert size(χ) == size(ζ) "Incompatible sizes"
  coords = Matrix{BoozerCoordinates}(undef,size(χ))
  map!((x,z)->BoozerCoordinates(ψ,x,z),coords,χ,ζ)
  return coords
end
...
function BoozerCoordinates(ψ::AbstractVector,χ::AbstractVector,ζ::AbstractVector)
  coords = Array{BoozerCoordinates,3}(undef,length(ζ),length(χ),length(ψ))
  for s = 1:length(ψ)
    for x = 1:length(χ)
      for z = 1:length(ζ)
        @inbounds coords[z,x,s] = BoozerCoordinates(ψ[s],χ[x],ζ[z])
      end
    end
  end
  return coords
...
function BoozerCoordinates(ψ::AbstractVector,χ::AbstractMatrix,ζ::AbstractMatrix)
  @assert size(χ) == size(ζ) "Incompatible sizes"
  coords = Array{BoozerCoordinates,3}(undef,(size(χ)...,length(ψ)))
  for s = 1:length(ψ)
    for j = 1:size(χ,2)
      for i = 1:size(χ,1)
        coords[i,j,s] = BoozerCoordinates(ψ[s],χ[i,j],ζ[i,j])
      end
    end
  end
  return coords
...
function BoozerCoordinates(ψ::AbstractArray{T,3},χ::AbstractArray{T,3},ζ::AbstractArray{T,3}) where T
  @assert size(ψ) == size(χ) == size(ζ) "Incompatible sizes"
  coords = Array{BoozerCoordinates,3}(undef,size(ψ))
  map!((s,x,z)->BoozerCoordinates(s,x,z),coords,ψ,χ,ζ)
  return coords
end

Base.show(io::IO, x::BoozerCoordinates) = print(io, "BoozerCoordinates(s=$(x.ψ), χ=$(x.χ), ζ=$(x.ζ))")
